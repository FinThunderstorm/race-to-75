...
test('GET /api/auth/me without a session returns 401', async ({ request }) => {
...
test('enrolling a passkey from a link logs the user in', async ({ page }) => {
  ...
  const optionsResponse = await page.request.post('/api/auth/enroll/options', {
    data: { token: rawToken }
  })
  ...
  const verifyResponse = await page.request.post('/api/auth/enroll/verify', {
...
test('POST /api/auth/enroll/options with a malformed body returns 400', async ({ request }) => {
  const response = await request.post('/api/auth/enroll/options', { data: { data: {} } })
...
test('a consumed enrollment token is rejected on second options request', async ({ page }) => {
  ...
  const firstOptions = await (
    await page.request.post('/api/auth/enroll/options', { data: { token: rawToken } })
  ).json()
  ...
  const secondOptions = await page.request.post('/api/auth/enroll/options', {
    data: { token: rawToken }
  })
...
test('a user can log out and log back in with the same passkey', async ({ page }) => {
  ...
  const loginVerify = await page.request.post('/api/auth/login/verify', {

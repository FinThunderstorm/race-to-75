...
test('admin UI invites a member who enrolls and gains management access after promotion', async ({
  ...
  const form = page.getByRole('form', { name: 'Invite user' })
  await form.getByLabel('Display name').fill('UI Invitee')
  await form.getByLabel('Email').fill(email)
  await form.getByRole('button', { name: 'Create invitation' }).click()
  const enrollmentUrl = await page
    .getByRole('textbox', { name: 'Enrollment link', exact: true })
    .inputValue()
  const memberContext = await browser.newContext({ baseURL })
  ...
  try {
    ...
    await memberPage.getByRole('button', { name: 'Create passkey' }).click()
    await expect(memberPage.getByText('Signed in as UI Invitee')).toBeVisible()
    await expect(memberPage.getByRole('link', { name: 'Manage users' })).toHaveCount(0)
    ...
    await card.getByRole('button', { name: 'Promote to admin' }).click()
    await expect(card.getByText('Admin', { exact: true })).toBeVisible()
    await memberPage.goto('/admin')
    await expect(
      memberPage.getByRole('heading', { name: 'Manage users', exact: true })
    ).toBeVisible()
    await card.getByRole('button', { name: 'Edit details' }).click()
    await card.getByLabel('Display name').fill('Updated Invitee')
    ...
    await card.getByRole('button', { name: 'Demote to member' }).click()
    ...
    await memberPage.getByRole('button', { name: 'Refresh users' }).click()
    await expect(memberPage).toHaveURL(`${baseURL}/`)

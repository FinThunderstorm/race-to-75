...
test('Eufy connection isolates profiles, imports once, survives reconnect, and stops after disconnect', async () => {
  ...
  try {
    ...
    expect(status.json().lastError).toContain('unavailable')
...
test('Eufy settings requires profile selection, clears credentials, and supports reconnect', async ({
  ...
  await eufy.getByRole('button', { name: 'Connect Eufy Life', exact: true }).click()
  await eufy.getByLabel('Eufy Life email').fill('eufy@example.com')
  await eufy.getByLabel('Eufy Life password').fill('secret')
  await eufy.getByRole('button', { name: 'Sign in to Eufy Life' }).click()
  await expect(eufy.getByLabel('Eufy Life password')).toHaveCount(0)
  await expect(eufy.getByRole('button', { name: 'Use this profile' })).toBeDisabled()
  await eufy.getByLabel('Your Eufy Life profile').selectOption('me')
  await eufy.getByRole('button', { name: 'Use this profile' }).click()
  await expect(eufy.getByText('Profile: Me')).toBeVisible()
  await expect(
    page
  ...
  await page.reload()
  await expect(eufy.getByText('Reconnect to resume syncing')).toBeVisible()
  await eufy.getByRole('button', { name: 'Reconnect Eufy Life' }).click()
  await expect(eufy.getByLabel('Eufy Life password')).toHaveValue('')
  await expect(eufy.getByLabel('Eufy Life email')).toHaveValue('')
  await eufy.getByRole('button', { name: 'Cancel' }).click()
  await page.setViewportSize({ width: 390, height: 844 })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await eufy.getByRole('button', { name: 'Disconnect Eufy Life' }).click()
  await expect(eufy.getByText('Not connected', { exact: true })).toBeVisible()
